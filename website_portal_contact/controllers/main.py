...
class WebsiteAccount(PortalController):
    def _contacts_domain(self, search=""):
        ...
        domain = request.env.ref("website_portal_contact.rule_edit_own_contacts").domain
    ...
    @route(["/my/contacts", "/my/contacts/page/<int:page>"], auth="user", website=True)
    def portal_my_contacts(self, page=1, date_begin=None, date_end=None, search=""):
    ...
    @route("/my/contacts/<model('res.partner'):contact>", auth="user", website=True)
    ...
    @route(
        "/my/contacts/<model('res.partner'):contact>/update", auth="user", website=True
    )
    ...
    @route(
        "/my/contacts/<model('res.partner'):contact>/disable", auth="user", website=True
    )
