...
class res_partner(models.Model):
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if self.env.context.get('search_show_all_positions', {}).get('is_set') and not self.env.context['search_show_all_positions']['set_value']:
            ...
            attached_contacts = super(res_partner, self).search(attached_contact_args)
            args = expression.OR((
                expression.AND(([('contact_type', '=', 'standalone')], args)),
                [('other_contact_ids', 'in', attached_contacts.ids)],
            ))
        return super(res_partner, self).search(args, offset=offset, limit=limit, order=order, count=count)
    ...
    def _commercial_partner_compute(self, name, args):
        ...
        result = super(res_partner, self)._commercial_partner_compute(name, args)
        for partner in self:
    ...
    def _fields_sync(self, partner, update_values):
        ...
        """
        super(res_partner, self)._fields_sync(partner, update_values)
        contact_fields = self._contact_fields()
...
class ir_actions_window(models.Model):
    _inherit = 'ir.actions.act_window'

    @api.multi
    def read(self, fields=None, context=None, load='_classic_read'):
        actions = super(ir_actions_window, self).read(fields=fields, load=load)
