...
class TestPartnerCountryLang(BaseCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner = cls.env["res.partner"].sudo().create({"name": "Mr Odoo"})

    def test_partner_create(self):
        partner = (
            self.env["res.partner"]
            .sudo()
            .create({"name": "Mrs Odoo", "country_id": self.country_fr.id})
        )
        self.assertEqual(partner.lang, self.lang_fr.code)

    def test_partner_write(self):
        self.partner.sudo().write({"country_id": self.country_fr.id})
