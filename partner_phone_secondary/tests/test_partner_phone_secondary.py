...
class TestPartnerPhoneSecondary(TransactionCase):
    ...
    def test_onchange_phone2_with_mock(self):
        partner = self.env["res.partner"].create({"name": "Test Partner"})
        mock_phone_format = Mock(return_value="+1 234 567 890")
        with patch.object(
            type(partner), "_phone_format", mock_phone_format, create=True
        ):
            partner.phone2 = "1234567890"
            partner._onchange_phone2_validation()
            mock_phone_format.assert_called_once_with(
                fname="phone2", force_format="INTERNATIONAL"
            )
            self.assertEqual(partner.phone2, "+1 234 567 890")
