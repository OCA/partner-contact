...
class Partner(models.Model):
    ...
    _rec_names_search = ['display_name_en', 'email', 'ref', 'vat',
    ...
    display_name = fields.Char(compute='_compute_display_name_proxy',
                               inverse='_set_display_name',
                               search='_search_display_name', store=False)
    display_name_en = fields.Char(compute='_compute_display_name_en',
                                  recursive=True,
                                  store=True,
                                  index='trigram')
    ...
    @api.depends('is_company', 'name', 'parent_id.display_name_en', 'type', 'company_name', 'commercial_company_name')
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        result = super(Partner, self).get_view(view_id=view_id, view_type=view_type, **options)
        lang_name = f'display_name_{self.env.user.lang.split("_")[0]}'
    ...
    def _force_display_names(self, vals):
        ...
        display_name_en = vals.get('display_name_en') or partner.with_context(dict(ctx, lang='en_US'))._get_name()
        partner.display_name_en = display_name_en
        # force backport compatible with display_name
        self.env.cr.execute(f"""UPDATE res_partner SET display_name='{display_name_en}' WHERE id={partner.id}""")

    # def _get_contact_name_with_context(self, partner, name):
    #     # fix pared name to get with current lan context not by superuser context
    #     ctx = self._context
    #     _logger.info("PARTNER CONTACT NAME %s\n%s %s %s" % (ctx, partner.with_context(ctx).commercial_company_name,
    #                                                         partner.with_context(ctx).sudo().parent_id.name, name))
    #     return "%s, %s" % (partner.with_context(ctx).commercial_company_name
    #                        or partner.with_context(ctx).sudo().parent_id.name, name)
