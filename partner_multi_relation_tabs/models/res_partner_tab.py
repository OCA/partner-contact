...
class ResPartnerTab(models.Model):
    ...
    code = fields.Char(
        string="Code", required=True, help="Language independent code for tab"
    )
    name = fields.Char(
        string="Name",
        required=True,
    ...
    contact_type = fields.Selection(
        selection="get_partner_types", string="Valid for partner type"
    )
    partner_category_id = fields.Many2one(
        comodel_name="res.partner.category", string="Valid for partner category"
    )
    ...
    def _check_partner_ids(self):
        ...
        if self.partner_ids and (self.contact_type or self.partner_category_id):
