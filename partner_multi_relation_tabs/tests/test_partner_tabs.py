...
class TestPartnerTabs(common.TestCommon):
    ...
    def test_create_tab(self):
        ...
        self.assertTrue(
            field,
            "Tab field %s does not exist in %s." % (fieldname, etree.tostring(tree)),
        )
        ...
        self.assertFalse(
            field,
            "Tab field %s should not exist in %s." % (fieldname, etree.tostring(tree)),
        )
    ...
    def test_tab_modifications(self):
        tab_executive = self.tab_model.create(
            {"code": "executive", "name": "Executive members"}
        )
        ...
        type_chairperson.write(
            {"contact_type_left": "c", "tab_left_id": tab_executive.id}
        )
        self.assertEqual(type_chairperson.tab_left_id.id, tab_executive.id)
        # Unlinking the tab should reset the tab_left_id on relation type.
        tab_executive.unlink()
        self.assertEqual(type_chairperson.tab_left_id.id, False)
    ...
    def test_relations(self):
        ...
        board_partners = relation_all_model.search([("tab_id", "=", self.tab_board.id)])
        ...
        self.assertEqual(
            board_partners.other_partner_id.id, self.partner_important_person.id
        )
    ...
    def test_compute_visibility(self):
        ...
        departments = [relation.other_partner_id for relation in department_relations]
        for department in departments:
            self.assertIn(self.category_department, department.category_id)
