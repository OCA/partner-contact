...
class TestCommon(common.SingleTransactionCase):
    ...
    def setUp(self):
        ...
        # Tabs.
        self.tab_committee = self.env.ref("partner_multi_relation_tabs.tab_committee")
        self.tab_board = self.env.ref("partner_multi_relation_tabs.tab_board")
        self.tab_positions = self.env.ref("partner_multi_relation_tabs.tab_positions")
        self.tab_departments = self.env.ref(
            "partner_multi_relation_tabs.tab_departments"
        )
        # Types.
        self.type_chairperson = self.env.ref(
            "partner_multi_relation_tabs" ".relation_type_committee_has_chairperson"
        )
        self.type_ceo = self.env.ref(
            "partner_multi_relation_tabs" ".relation_type_company_has_ceo"
        )
        # Partners.
