...
# Copyright 2022 PT Solusi Aglis Indonesia - Panca Putra Pakpahan
...
class ResPartner(models.Model):
    ...
    @api.constrains('mobile', 'company_id')
    def _check_mobile_unique(self):
        for record in self:
            if record.parent_id or not record.mobile:
                continue
            test_condition = (config['test_enable'] and
                              not self.env.context.get('test_mobile'))
            if test_condition:
                continue
            if self.env['res.partner'].sudo().with_context(
                ...
                raise ValidationError((_(
                    "The Mobile %s already exists in another "
                    "partner."
                ) + " " + _(
