...
class PersonCase(TransactionCase):
    ...
    def tearDown(self):
        try:
            new = (self.env[self.model].with_context(self.context).create(
                self.params))

            # Check that each individual field matches
            self.assertEqual(self.firstname, new.firstname,
                             "First name saved badly.")
            self.assertEqual(self.lastname, new.lastname,
                             "Last name 1 saved badly.")
            self.assertEqual(self.lastname2, new.lastname2,
    ...
    def test_firstname_first(self):
        ...
        self.params = {
            "is_company": False,
            "name":
            "%s %s %s" % (self.firstname, self.lastname, self.lastname2),
        }
    ...
    def test_firstname_last(self):
        ...
        self.params = {
            "is_company": False,
            "name":
            "%s %s, %s" % (self.lastname, self.lastname2, self.firstname),
        }
